...
binary. Binary sequences often indicate occurrence of random events:
* Extreme returns on financial markets.
* Value-at-risk violations.
...
need take a close look at the dependence structure of the sequence under
consideration.

#### What are we going to learn here?

In this blog post, I am going to formulate and investigate the question of
serial (or temporal) dependence in random binary sequences. We will learn about
the statistical approach to this problem and about a dependence score based
...
To set an appropriate context for the serial depencende detection problem, let
...
We often tend to see serial depencende in situations where there is clearly
none. Typical examples are "winning or lucky streaks" experienced by gamblers
in casinos.

In order to rigorously investigate the question of serial dependence in the
...
One way of test whether a waiting times sequence follows a geometric
...
distribution with the parameter $$p$$, the above scalar product takes the form
...
$$(q_i(x))_{i\geq 0}$$ is called _orthogonal if and only if $$\langle q_k, q_i \rangle = 0$$
for all $$k \neq i$$, each $$q_i$$ has degree $$i$$.
As a consequence we have $$\mathbb E q_i (Y) = 0$$. This is the tool I am
going to use to check whether $$Y$$ follows a geometric distribution.

The family of orthogonal polynomials $$M = (M_i(x))_{i\geq 0}$$ corresponding
to the geometric distribution is a special case of the _Meixner family_ derived
from the negative binomail distribution (a generalization of the geometric
...
for a geometric random variable $$Y$$. This relation can be used to test
whether a given sample of waiting times belongs to a geometric distribution
...
interpreted as an evidence speaking for geometric distribution with
...
zero can also occurr if the events are independend but the true event
probabilty $$\tilde p$$ differs significantly from our assumptions. This
...
serial dependence in the sense developed above, we can define the *Meixner
dependency score (MDS)* as a mean of expectation estimates for the first $$k$$
Meixner polynomials evalualted at a sample of waiting times 
$$x = (x_1,..,x_m)$$: 

$$D^k_{\text{meixner}}(x) = \frac{1}{k} \sum_{i=1}^{k} |\bar{M}_i(x)|.$$ 

Remark: The idea of using orthogonal polynomials to test distibutional
assumptions has its roots in the method of moments and was developed in the
...
To test whether the procedure devised above has a chance to work in practice, I
...
For a model with a simple form of serial dependence consider the following:
We select probabilities $$0 < p_1 < p_0 < p_2 < 1$$ and set the distributions
...
distributions are $$X_i\sim\text{Ber}(p_0)$$.
...
sequences. We implemented a serial dependence detection method based
on Meixner orthogonal polynomials and tested it's performance on a simple
Markov chain model.
...
and the initial distribution $$\lambda = (1-p0, p0)$$. The marginal
...
task of finding a $$p_2$$ such that $$\mathbb P (X_{i} = 1)$$ for all $$i>0$$ is as close
